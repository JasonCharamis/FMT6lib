...
class FMT6:
    ...
    def extract_genes ( fmt6_file, gene_list, query = True   ):

        hits = []
        
        glist = isfile(gene_list)
        fmt6_instances = FMT6.parse_fmt6 ( fmt6_file )
...
def main():
    ...
    if args.fmt6:
        ...
        if args.sort:
            ...
            if args.gene_list:
                with open(f"{inp}.{args.gene_list}.fmt6", "w") as f:
                    print ( "Extracting", args.gene_list, "from", args.fmt6 )

                    for out in FMT6.extract_genes(args.fmt6, args.gene_list, query=args.query):
                        print ( out, file = f )
            else:
